...
| **fanta.bio MCP** | functional genomics; cis-regulatory elements; transcription factor binding; gene expression | `https://mcp.fanta.bio/mcp` | fanta.bio | official-public | Streamable HTTP (JSON-RPC 2.0), no auth (per-endpoint hourly rate limits: 500/hr lookups, 100/hr full-text, 50/hr batch). CREs, genes, SNPs and TF binding for human (hg38) / mouse (mm10), cross-referenced with SCREEN cCRE / ChIP-Atlas / Ensembl / GWAS. 19 tools, 5 prompts, 2 resources. Docs at `docs.fanta.bio/mcp/`; transport endpoint `mcp.fanta.bio/mcp`. Source: [fanta.bio MCP docs](https://docs.fanta.bio/mcp/) |
